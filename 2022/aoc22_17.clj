...
(defn tower-height
  "Вычисляет высоту башни из камней. Момент измерения определяется значением
...
(let [target-shape-no 1000000000001
      winds (mapv (comp keyword str) data)
      ;; Сначала находим, когда в первый раз повторится начальная конфигурация, т.е. когда при появлении
      ;; первого камня будет первый же элемент из списка направлений ветра.
      ...
      ;; будет цикличным, т.к. на самой первой итерации построение башни камней идет относительно ровной поверхности
      ;; пола. А вот уже после второго повторения высота будет прирастать строго циклично.
      [height2 shape-no2]
      (tower-height winds shapes (fn [shape-no iter]
                                   (and (> iter iter1)
                                        (zero? (rem shape-no (count shapes)))
